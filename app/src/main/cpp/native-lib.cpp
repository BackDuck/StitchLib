...
extern "C" {
void stch(Stitcher &stitcher, Mat &result, vector<Mat> &natImg) {
    stitcher.stitch(natImg, result);
}
JNIEXPORT void JNICALL
Java_com_example_nurshat_stitchlib_NativeStitcherWrapper_NativeStitch(JNIEnv *env, jobject thisObj,
                                                                      jobjectArray jInputArray,
                                                                      jlong jResultMat,
...
//    stitcher.stitch(natImgs, result);
    std::thread first(stch, std::ref(stitcher), std::ref(result), std::ref(natImgs));
    first.join();
